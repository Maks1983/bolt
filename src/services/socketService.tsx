...
export class WebSocketService {
  ...
  constructor() {
    console.log('📋 Subscribed entities:', subscribedEntities.length);
      ...
      case 'event':
        ...
        if (message.event?.event_type === 'state_changed') {
          ...
          if (eventData?.new_state && this.isSubscribedEntity(eventData.entity_id)) {
            console.log('🔄 Processing subscribed entity update:', eventData.entity_id);
  ...
  /**
   * Check if entity is in our subscription list
   */
  private isSubscribedEntity(entityId: string): boolean {
    return subscribedEntities.some(entity => entity.entity_id === entityId);
  }
  ...
  private getStates(): void {
    ...
    this.pendingMessages.set(getStatesMessage.id!, (response) => {
      if (response.success && response.result) {
        ...
        const subscribedEntityIds = subscribedEntities.map(e => e.entity_id);
        const filteredStates = response.result.filter((state: any) => 
          subscribedEntityIds.includes(state.entity_id)
        );
        
        console.log(`📊 Received ${response.result.length} total states, ${filteredStates.length} are subscribed`);
  ...
  private convertHAStatesToDevices(states: any[]): Device[] {
    return states.map(state => {
      ...
      console.log('✅ Converted device:', device.friendly_name, 'State:', device.state);
      return device;
