...
// CRITICAL: Fix for default markers in react-leaflet
...
// Component to handle map centering and tile loading
const MapController: React.FC<{ center: [number, number]; onReady: () => void }> = ({ center, onReady }) => {
  const map = useMap();
  
  useEffect(() => {
    // Set view and force map to invalidate size
    map.setView(center, map.getZoom());
    
    // Force map to redraw and load tiles properly
    setTimeout(() => {
      map.invalidateSize();
...
const LocationModal: React.FC<LocationModalProps> = ({ user, onClose }) => {
  ...
  const handleMapReady = () => {
    console.log('🗺️ Map container ready, initializing tiles...');
    setMapReady(true);
  };
          ...
          <div className="p-6">
            <div className="seamless-card rounded-2xl h-96 overflow-hidden relative">
              ...
              <MapContainer
                ...
                preferCanvas={false}
              >
                {/* OpenStreetMap Tile Layer with proper configuration */}
                <TileLayer
                ...
                {/* Map Controller */}
                {mapReady && <MapController center={mapCenter} onReady={handleTilesLoaded} />}
              </MapContainer>

              {/* Loading overlay */}
              {(!mapReady || !tilesLoaded) && (
                <div className="absolute inset-0 seamless-card flex items-center justify-center rounded-2xl z-10">
                  <div className="text-center">
