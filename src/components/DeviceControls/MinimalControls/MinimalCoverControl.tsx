    ...
    <div className="bg-gray-700 rounded-xl p-4 border border-gray-600 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className={`p-2 rounded-lg ${isOpen ? 'bg-blue-900/50' : 'bg-gray-600'}`}>
            <Columns2 className={`w-5 h-5 ${isOpen ? 'text-blue-400' : 'text-gray-400'}`} />
          </div>
        ...
        <button
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            isOpen ? 'bg-blue-600' : 'bg-gray-500'
          }`}
        >
